...
def get_artists(url):
    ...
    soup = BeautifulSoup(r.text, 'html5lib')3                  
    artist_ids = []
    for artist in soup.find_all('a', attrs={'class': 'nm nm-icn f-thide s-fc0'}):
        ...
        artist_ids.append(artist['href'].replace('/artist?id=', '').strip())
    getArtistsHotSongs.get_Artists_HotSongs(artist_ids)
...
# ls2 = [-1]    # initial的值
# csvfile = open('artists_all_IdAndName.csv', 'a', newline='',encoding='utf-8')    # 文件存储的位置
